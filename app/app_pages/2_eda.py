...
def run():
    ...
    species_options = ["All species"] + sorted(df["Species"].dropna().unique())
    selected_species = st.selectbox("Select species to view monthly collisions:", species_options)

    df_selected = df if selected_species == "All species" else df[df["Species"] == selected_species]
    ...
    *Interpretation:*  
    ...
    species_for_time = st.selectbox("Select species to view hourly collisions:", species_options)

    df_time = df if species_for_time == "All species" else df[df["Species"] == species_for_time]
    ...
    *Interpretation:*  
    ...
    selected_species_3 = st.selectbox("Select species to view collision distribution by county:", species_options)

    df_county = df if selected_species_3 == "All species" else df[df["Species"] == selected_species_3]
    county_counts = df_county["County"].value_counts().sort_values(ascending=False)
    ...
    *Interpretation:*  
    ...
    fig_map = px.scatter_mapbox(
        ...
        zoom=4.5,
        height=600,
        title=f"Wildlife Collisions in {selected_year} ({selected_species_map})"
    )
